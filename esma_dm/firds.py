...
class FIRDSClient:
    ...
    def reference(self, isin: str) -> Optional[pd.Series]:
        ...
        Searches through downloaded FIRDS data files to find the instrument.
        If not found in latest files, searches through all available cached files.
        ...
        Example:
            ...
            >>> 
            >>> if ref is not None:
            ...     print(f"Name: {ref.get('FullNm')}")
            ...     print(f"CFI: {ref.get('ClssfctnTp')}")
            ...     print(f"Currency: {ref.get('NtnlCcy')}")
        """
        # Validate ISIN format
        if not self.validate_isin(isin):
            raise ValueError(f"Invalid ISIN format: {isin}")
        
        # First, try to fetch from latest files via API
        df = self.get_instruments([isin])
        
        if not df.empty:
            return df.iloc[0]
        
        # If not found, search through cached files
        self.logger.info(f"Searching cached files for ISIN: {isin}")
        
        # Get cache directory
        cache_dir = self.config.downloads_path / 'firds'
        
        if not cache_dir.exists():
            self.logger.warning(f"No cache directory found at {cache_dir}")
            return None
        
        # Search through all CSV files in cache
        import glob
        csv_files = glob.glob(str(cache_dir / '*.csv'))
        
        for csv_file in csv_files:
            try:
                # Read CSV and look for ISIN
                df = pd.read_csv(csv_file)
                
                if 'Id' in df.columns:
                    match = df[df['Id'] == isin]
                    if not match.empty:
                        self.logger.info(f"Found ISIN in cached file: {csv_file}")
                        return match.iloc[0]
            except Exception as e:
