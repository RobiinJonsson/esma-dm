...
class BulkInserter:
    ...
    def insert_swaps(self, df: pd.DataFrame):
        ...
        delivery_col = self._find_column(df, ['DlvryTp', 'delivery_type'])
        ir_ref_col = self._find_column(df, ['IntrstRate_Ref_Nm'])
        ir_term_unit_col = self._find_column(df, ['IntrstRate_Trm_Unit'])
        ir_term_value_col = self._find_column(df, ['IntrstRate_Trm_Val'])
